...
def create_cart(cart_data : AllCart):
    ...
    total_price = cart_data.quantity*product.product_price
    
    cart_item = db.query(Cart).filter( Cart.user_id == cart_data.user_id,Cart.product_id == cart_data.product_id ).first()
...
#remove cart
@carts.post("/remove_from_cart", response_model=AllCart)
def remove_from_cart(cart_data: AllCart):
    cart_items = db.query(Cart).filter(Cart.user_id == cart_data.user_id, Cart.product_id == cart_data.product_id ).first()
